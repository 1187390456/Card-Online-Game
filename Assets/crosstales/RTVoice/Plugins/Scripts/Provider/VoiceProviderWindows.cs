...
namespace Crosstales.RTVoice.Provider
{
    ...
    public class VoiceProviderWindows : BaseVoiceProvider
    {
                ...
                else
                {
                    ...
                    if (System.IO.File.Exists(application))
                    {
                        ...
                        using (System.Diagnostics.Process speakProcess = new System.Diagnostics.Process())
                        {
                            ...
                            System.Threading.Thread worker = new System.Threading.Thread(() => readSpeakNativeStream(speakProcess, speechTextArray, out wordIndex, out phoneme, out viseme, out start)) { Name = wrapper.Uid.ToString() };
                            worker.Start();
...
#if (UNITY_STANDALONE_WIN || UNITY_EDITOR) && !UNITY_WEBPLAYER
        private void readSpeakNativeStream(System.Diagnostics.Process process, string[] speechTextArray, out int wordIndex, out string phoneme, out string viseme, out bool start)
        {
            string reply;
        ...
        private string applicationName()
        {
            ...
            {
                if (Util.Constants.ENFORCE_32BIT_WINDOWS)
                {
                    return dataPath + Util.Constants.TTS_WINDOWS_EDITOR_x86;
                }
                else
                {
                    return dataPath + Util.Constants.TTS_WINDOWS_EDITOR;
                }
            }
            else
            {
                return dataPath + Util.Constants.TTS_WINDOWS_BUILD;
            }
        }
        ...
        public override void GenerateInEditor(Model.Wrapper wrapper)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        Debug.LogError("Could not find the TTS-wrapper: '" + application + "'");
                    }
                }
            }
#endif
